...
- 지게차는 패딩 추가 후 BFS로 외부에서 접근 가능한 컨테이너 제거 / 크레인은 전체 순회해서 해당 문자 제거

배운 점: 

- BFS 시작점을 통일하기 위해 패딩(빈 테두리) 추가 / '.' + string은 char ASCII 값이 더해져서 오동작 → "." + string 써야 함
