...
---
- 풀이: DP / dp[i] = B 흔적이 i일 때 A 흔적 최솟값 / 물건마다 A가 훔치는 경우와 B가 훔치는 경우로 next 배열 갱신
- 배운 점: DP에서 next 배열을 따로 만들어야 같은 물건을 두 번 쓰는 걸 방지 / INT_MAX로 불가능한 상태 표현 / dp[0]=0으로 시작 상태 초기화 필수
- 실수: dp 크기를 n으로 선언 → m으로 수정 / dp[0]=0 초기화 누락
