...
- 전체 배열을 한 번 순회하며 '#'의 행/열 min/max를 추적
- 드래그 끝점은 마지막 파일의 다음 칸이므로 maxR, maxC에 +1

배운 점:

- 행/열 변수명을 r/c, rows/cols로 명확히 구분하면 인덱스 혼동을 방지할 수 있음 (다음부터 그렇게 할 예정)
- 좌표 범위가 정해진 경우 INT_MAX보다 rows, cols로 초기화하는 게 의도가 더 잘 드러남
- early continue를 쓰면 중첩 if보다 로직 흐름이 선형적으로 읽힘

실수:

- W/H와 x/y가 동시에 뒤바뀌어 배열 접근은 동작했지만 반환값의 행·열 순서가 역전됨
- 정사각형 테스트케이스는 통과하고 직사각형에서 틀리는 버그라 찾기 어려웠음
