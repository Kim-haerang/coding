...
풀이:
- 시간을 초 단위로 변환해서 계산
- prev/next 명령에 따라 ±10초 이동 후 범위 보정
- 현재 위치가 오프닝 구간이면 op_end로 이동

배운 점:
- 시간 계산은 하나의 단위로 통일하는 게 중요
- 이동 후 범위 보정과 오프닝 체크 순서가 중요

실수:
- 초 단위를 문자열로 바꿀 때 /100을 사용함 (/60이 맞음)
- 시작 위치가 오프닝 구간인 경우를 처음에 놓침
