...
---
풀이:
- arr 풀이: 공원 정보를 빈칸 여부 배열로 바꾼 뒤, 각 위치에서 m x m 영역이 모두 비어 있는지 확인
- dp 풀이: 각 칸을 오른쪽 아래 끝으로 하는 최대 정사각형 크기를 계산
- mats를 큰 순서로 확인해서 놓을 수 있는 가장 큰 돗자리 크기 반환

배운 점:
- 2차원 배열에서는 시작점 기준 완전탐색과 끝점 기준 DP 모두 가능
- DP에서 `dp[i][j]`의 의미를 정확히 정하는 것이 중요
- 정사각형 확장은 위, 왼쪽, 왼쪽 위 값 중 최솟값을 이용한다

실수:
- 배열 전체를 `if (arr_park)`처럼 검사하려고 함
- `arr_park[i][j]`처럼 현재 칸을 확인해야 했음
- DP에서 현재 칸을 오른쪽 아래 끝으로 본다는 점이 헷갈렸음
