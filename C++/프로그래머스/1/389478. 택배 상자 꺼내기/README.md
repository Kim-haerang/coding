...
---
- 풀이: 뱀처럼 쌓인 구조에서 num의 열 위치를 수식으로 계산 후 위층 순회
- 배운 점: 프로그래머스 기본 템플릿 헤더는 꼭 써야 하는 게 아님, 짝수/홀수 행마다 열 계산 방향이 반대
- 실수: index에 상자 번호를 저장함 (열 위치를 저장했어야 함), `b[index]` 로 존재 여부만 체크함 (같은 열인지 확인했어야 함)
