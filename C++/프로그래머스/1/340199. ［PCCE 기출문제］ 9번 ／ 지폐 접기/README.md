...
---
풀이:
- 지폐의 큰 쪽과 작은 쪽을 나눠서 지갑에 들어가는지 확인
- 지갑은 회전해서 넣을 수 있으므로 두 방향 모두 비교
- 들어가지 않으면 지폐의 더 긴 쪽을 반으로 접음

배운 점:
- 가로/세로가 있는 문제는 회전 가능 여부를 고려해야 함
- 큰 쪽을 줄이는 방식으로 반복하면 간단하게 해결 가능

실수:
- 처음에는 지갑과 지폐의 방향을 고정해서 비교함
- 회전해서 들어가는 경우를 놓침
