...
- section이 정렬된 채로 주어지므로 map 없이 greedy로 풀 수 있음
- 마지막으로 칠한 위치를 추적하며, 현재 빈 구간이 범위 밖이면 한 번 더 칠함

배운 점:

- 입력 조건(정렬 여부, 범위 등)을 먼저 확인하면 자료구조 선택이 단순해짐
- 상태 전체를 map으로 관리하는 것보다 필요한 값 하나만 추적하는 greedy가 효율적인 경우가 많음

실수:

- 전체 벽을 unordered_map으로 관리하려다 불필요하게 복잡해짐
- map에 n 범위를 벗어난 키가 추가되어 루프가 의도보다 길게 돌아가는 버그 발생
